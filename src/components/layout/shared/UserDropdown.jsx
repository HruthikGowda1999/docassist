...
const UserDropdown = () => {
  ...
  useEffect(() => {
    ...
    const fullnameCookie = cookies.find(cookie => cookie.startsWith('user_fullname'))
    ...
    if (fullnameCookie) {
      try {
        setFullName(decodeURIComponent(fullnameCookie.split('=')[1] || ''))
      } catch (err) {
        console.error('Failed to parse user cookie:', err)
      }
    }
